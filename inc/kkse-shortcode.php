...
final class Kkse_shortcode {
	...
	public function add_shortcode_bilde($atts, $content = null) {
		...
		$meta = get_post_meta($post[0]->ID, 'emkredittkortse_data');
		...
		if ($meta['bestill']) {
			...
			$html .= '<div class="emkredittkort-logo-ls"'.($float ? $float : '').'><a target="_blank" rel=noopener href="'.esc_url($meta['bestill']).'"><img alt="'.esc_attr($post[0]->post_title).'" src="'.esc_url(get_the_post_thumbnail_url($post[0], 'full')).'"></a></div>';
			return $html;
		}
		// anchor-less image
		return'<div class="emkredittkort-logo-ls"'.($float ? $float : '').'><img alt="'.esc_attr($post[0]->post_title).'" src="'.esc_url(get_the_post_thumbnail_url($post[0], 'full')).'"></div>';
	}
	...
	public function add_shortcode_bestill($atts, $content = null) {
		...
		$html .= '<div class="emkredittkort-bestill emkredittkort-bestill-mobile"'.($float ? $float : '').'><a target="_blank" rel="noopener" class="emkredittkort-link" href="'.esc_url($meta['bestill']).'"><svg class="emkredittkort-svg" version="1.1" x="0px" y="0px" width="26px" height="20px" viewBox="0 0 26 20" enable-background="new 0 0 24 24" xml:space="preserve"><path fill="none" d="M0,0h24v24H0V0z"/><path class="emkredittkort-thumb" d="M1,21h4V9H1V21z M23,10c0-1.1-0.9-2-2-2h-6.31l0.95-4.57l0.03-0.32c0-0.41-0.17-0.79-0.44-1.06L14.17,1L7.59,7.59C7.22,7.95,7,8.45,7,9v10c0,1.1,0.9,2,2,2h9c0.83,0,1.54-0.5,1.84-1.22l3.02-7.05C22.95,12.5,23,12.26,23,12V10z"/></svg> Ansök här!</a></div>';
		return $html;
	}
	...
	public function add_css() {
        wp_enqueue_style('emkredittkortse-style', KREDITTKORT_SE_PLUGIN_URL.'assets/css/pub/em-kredittkort-se.css', array(), '1.0.1', '(min-width: 801px)');
        wp_enqueue_style('emkredittkortse-mobile', KREDITTKORT_SE_PLUGIN_URL.'assets/css/pub/em-kredittkort-se-mobile.css', array(), '1.0.1', '(max-width: 800px)');
	}
	...
	private function get_html($posts, $atts) {
		$html = '<ul class="emkredittkort-ul">';

		foreach ($posts as $p) {
			
			$meta = get_post_meta($p->ID, 'emkredittkortse_data');
			...
			// grid container
			$html .= '<li class="emkredittkort-container">';
			...
			// info container
			$html .= '<div class="emkredittkort-info-container">';

			// info 1
			if ($meta['info01']) $html .= '<div class="emkredittkort-info emkredittkort-info-en">'.$meta['info01'].'</div>';

			// info 2
			if ($meta['info02']) $html .= '<div class="emkredittkort-info emkredittkort-info-to">'.$meta['info02'].'</div>';

			// info 3
			if ($meta['info03']) $html .= '<div class="emkredittkort-info emkredittkort-info-tre">'.$meta['info03'].'</div>';

			// info 4
			if ($meta['info04']) $html .= '<div class="emkredittkort-info emkredittkort-info-fire">'.$meta['info04'].'</div>';

			$html .= '</div>';

			// info list container 
			$html .= '<div class="emkredittkort-list-container">';

			// info 5
			if ($meta['info05']) $html .= '<div class="emkredittkort-info emkredittkort-info-fem">'.$meta['info05'].'</div>';

			// info 6
			if ($meta['info06']) $html .= '<div class="emkredittkort-info emkredittkort-info-seks">'.$meta['info06'].'</div>';

			// info 7
			if ($meta['info07']) $html .= '<div class="emkredittkort-info emkredittkort-info-syv">'.$meta['info07'].'</div>';

			// info 8
			if ($meta['info08']) $html .= '<div class="emkredittkort-info emkredittkort-info-atte">'.$meta['info08'].'</div>';

			$html .= '</div>';

			$html .= '<div class="emkredittkort-end-container">';
			// terning
			if ($meta['terning'] != 'ingen') {
				$html .= '<svg class="emkredittkort-terning">
							<defs>
							    <linearGradient id="emkredittkort-grad" x1="0%" y1="100%" x2="100%" y2="100%">
							      <stop offset="0%" style="stop-color:rgb(200,0,0);stop-opacity:1" />
							      <stop offset="100%" style="stop-color:rgb(255,0,0);stop-opacity:1" />
							    </linearGradient>
							  </defs>
							<rect class="emkredittkort-rect-svg" rx="7" ry="7" fill="url(#emkredittkort-grad)"/>';

				switch ($meta['terning']) {

					case 'seks':
					$html .= '<circle class="emkredittkort-circle-svg" cx="11" cy="25" r="5"/>';
					$html .= '<circle class="emkredittkort-circle-svg" cx="39" cy="25" r="5"/>';

					case 'fire':
					$html .= '<circle class="emkredittkort-circle-svg" cx="11" cy="10" r="5"/>';
					$html .= '<circle class="emkredittkort-circle-svg" cx="39" cy="40" r="5"/>';

					case 'to':
					$html .= '<circle class="emkredittkort-circle-svg" cx="11" cy="40" r="5"/>';
					$html .= '<circle class="emkredittkort-circle-svg" cx="39" cy="10" r="5"/>';
					break;

					case 'fem':
					$html .= '<circle class="emkredittkort-circle-svg" cx="10" cy="10" r="5"/>';
					$html .= '<circle class="emkredittkort-circle-svg" cx="40" cy="40" r="5"/>';

					case 'tre':
					$html .= '<circle class="emkredittkort-circle-svg" cx="10" cy="40" r="5"/>';
					$html .= '<circle class="emkredittkort-circle-svg" cx="40" cy="10" r="5"/>';

					case 'en':
					$html .= '<circle class="emkredittkort-circle-svg" cx="25" cy="25" r="5"/>';
					break;
			...
			// bestill 
			$html .= '<div class="emkredittkort-bestill-container">';
			// $html .= '<div class="emkredittkort-bestill"><a target="_blank" rel="noopener" class="emkredittkort-link" href="'.$meta['bestill'].'">Ansök här <svg class="emkredittkort-svg" version="1.1" x="0px" y="0px" width="26px" height="20px" viewBox="0 0 26 22" enable-background="new 0 0 24 24" xml:space="preserve"><path fill="none" d="M0,0h24v24H0V0z"/><path opacity="0.87" fill="none" d="M0,0h24v24H0V0z"/><path fill="#fff" d="M18,8h-1V6c0-2.76-2.24-5-5-5S7,3.24,7,6v2H6c-1.1,0-2,0.9-2,2v10c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2V10C20,8.9,19.1,8,18,8z M9,6c0-1.66,1.34-3,3-3s3,1.34,3,3v2H9V6z M18,20H6V10h12V20z M12,17c1.1,0,2-0.9,2-2c0-1.1-0.9-2-2-2c-1.1,0-2,0.9-2,2C10,16.1,10.9,17,12,17z"/></svg></a></div>';
			$html .= '<div class="emkredittkort-bestill"><a target="_blank" rel="noopener" class="emkredittkort-link" href="'.$meta['bestill'].'"><svg class="emkredittkort-svg" version="1.1" x="0px" y="0px" width="26px" height="20px" viewBox="0 0 26 20" enable-background="new 0 0 24 24" xml:space="preserve"><path fill="none" d="M0,0h24v24H0V0z"/><path class="emkredittkort-thumb" d="M1,21h4V9H1V21z M23,10c0-1.1-0.9-2-2-2h-6.31l0.95-4.57l0.03-0.32c0-0.41-0.17-0.79-0.44-1.06L14.17,1L7.59,7.59C7.22,7.95,7,8.45,7,9v10c0,1.1,0.9,2,2,2h9c0.83,0,1.54-0.5,1.84-1.22l3.02-7.05C22.95,12.5,23,12.26,23,12V10z"/></svg> Ansök här!</a></div>';
			$html .= '<div class="emkredittkort-bestilltext">'.$meta['bestill_text'].'</div>';
			$html .= '</div>';
	...
	private function add_query_string($link = null, $source = null, $page = null) {
		...
		else $def = $adservice; // adservice is currently default
	...
	public function add_serp($data) {
		...
		if ($post->post_type != 'emkredittkortse') return $data;
